...
WiFiTransport.prototype.defineIgnorePins = function () {
  ...
  switch (this.controller) {
    ...
    case Controllers.ESP8266:
      text += "#if defined(ESP8266_WIFI) && defined(SERIAL_DEBUG)\n";
      // ignore SPI pins, pin 4 (SS for SD-Card on WiFi-shield), pin 7 (WiFi handshake)
      // and pin 10 (WiFi SS)
...
WiFiTransport.prototype.createConfigBlock = function () {
  ...
  if (configuration.serverIp) {
    if (utils.validateIp(configuration.serverIp)) {
      config += "// IP address of remote server\n";
      config += "#define SERVER_IP " + configuration.serverIp.split(".").join(", ") + "\n\n";
    } else {
  ...
  if (configuration.serverIp) {
    ...
    config += "WiFiClientStream stream(IPAddress(" + configuration.serverIp.split(".").join(", ") + "), SERVER_PORT);\n\n";
  } else {
    // configure board as a TCP server
    config += "WiFiServerStream stream(SERVER_PORT);\n\n";
  }
