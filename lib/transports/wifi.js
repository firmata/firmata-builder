...
WiFiTransport.prototype.createIgnorePinsFn = function () {
  ...
  switch (this.controller) {
  case Controllers.WIFI_SHIELD:
    fn += "    if (IS_IGNORE_WIFI_SHIELD(i)\n";
    fn += "#if defined(__AVR_ATmega32U4__)\n";
    fn += "      || 24 == i // on Leonardo, pin 24 maps to D4 and pin 28 maps to D10\n";
    fn += "      || 28 == i\n";
    fn += "#endif\n";
    fn += "    ) {\n";
    break;
  case Controllers.WIFI_SHIELD_101:
  case Controllers.MKR1000:
    fn += "    if (IS_IGNORE_WIFI101_SHIELD(i)) {\n";
    break;
  default:
    fn += "    if (false) {\n";
  }
...
WiFiTransport.prototype.createInitTransportFn = function () {
  ...
  if (this.controller === Controllers.WIFI_SHIELD) {
    fn += "  DEBUG_PRINTLN(\"" + attempting + " using the legacy WiFi library.\");\n";
  } else if (this.controller === Controllers.WIFI_SHIELD_101 ||
             this.controller === Controllers.MKR1000) {
    fn += "  DEBUG_PRINTLN(\"" + attempting + " using the WiFi 101 library.\");\n";
  }
  ...
  fn += "\n";
  fn += "  DEBUG_PRINTLN(\"WiFi setup done\");\n";
  fn += "  printWiFiStatus();\n\n";
